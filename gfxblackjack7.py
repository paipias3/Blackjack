...
class MainWindow(wx.Frame):
    def __init__(self, parent, title, width, height):
        ...
        Gamehelper = wx.Button(self.titleScreen, -1, label='Game Helper', pos=(30, 100))
        Gamehelper.Bind(wx.EVT_BUTTON, self.newGamehelper)

    def newGamehelper(self, e):
        self.gamehelper = GameHelper(None, self, 'Game Helper', 800, 600)
        self.gamehelper.Show(True)
...
class GameHelper(wx.Frame):
    ''' The popup that appears that asks for the game parameters (player names, number of players).'''
    def __init__(self, parent, mainWindow, title, width, height):
        wx.Frame.__init__(self, parent, title=title, size=(width, height), style=wx.SYSTEM_MENU | wx.MINIMIZE_BOX | wx.CAPTION | wx.CLOSE_BOX | wx.CLIP_CHILDREN)
        self.newGameScreen = wx.Panel(self)

        self.Gamehelper = wx.StaticText(self.newGameScreen, label="The rule first determines the bet amount. \nThe dividend rate is based on the amount you get. \nIf you win 100 won and lose 200 won, you will lose 100 won.\n\n The number of the card is counted as it is on the card.\n It is a simple game to make 21 by adding these numbers.\n K, Q, and J correspond to 10, and an ace can be either 1 or 11.\n\n You will receive two basic cards,\n and you can combine the numbers of the cards to make it as close to 21 as possible.\n If the first two cards are combined, 21 will be blackjack.\n If it is not 21, you can draw any number of cards if you want.\n If it exceeds 21 (22 or more),\n it is called 'burst' and you lose the unconditionally amount regardless of the result of the dealer.", pos=(20, 18))
...
class Table(wx.Panel):
        ...
        if showAnnouncement:
            ...
            if currentPlayer.name is 'Dealer':
                ...
                if currentPlayer.state is 'playing':
                    bettingbutton = wx.Button(self, -1, label='raise', pos=(300, 240))
                    bettingbutton.Bind(wx.EVT_BUTTON, self.playerBetting)
                    self.bettingMoney = wx.StaticText(self, label=str(self.betting), pos=(200, 240))
                    self.bettingMoney.SetFont(self.NAMEFONT)
                    self.bettingMoney.SetForegroundColour(self.NAMECOLOUR)
            ...
            if player.name is not 'Dealer':
                if player.state is 'standing' or 'busted':
                    if player.state == 'busted':
                        result = '%s loses $%i (Bust)' % (player.name, self.betting)
                        player.money -= self.betting
                    elif player.calculatePoints() < self.dealer.calculatePoints() and self.dealer.calculatePoints() <= 21:
                        result = '%s loses $%i (%i points)' % (player.name, self.betting, player.calculatePoints())
                        player.money -= self.betting
                    elif player.calculatePoints() == self.dealer.calculatePoints() and self.dealer.calculatePoints() <= 21:
                        result = '%s pushes' % player.name
                    elif player.calculatePoints() == 21 and len(player.hand) < 3:
                        result = '%s wins $%i (Blackjack)' % (player.name, self.betting)
                        player.money += self.betting
                        dealerNetGain -= self.betting
                    else:
                        result = '%s wins $%i (%i points)' % (player.name, self.betting, player.calculatePoints())
                        player.money += self.betting
                        dealerNetGain -= self.betting
    ...
    def playerBetting(self, e):
        if self.betting < self.playerList[self.currentPlayer].money:
            self.betting += 100
        self.renderTable(wx.EVT_PAINT)
